...
class SampleAppTools(BaseTools):
    def __init__(self):
        super().__init__(owner="sample_app", log_label="sample_app_gen", logger=logger)
    ...
    def docker_build(
        ...
        with timed_step(logger, "sample_app_gen.tools.docker_build", image_name=image_name, tag=tag):
            ...
            error = self._validate_docker_target(target, image_name, tag, image_ref, command_label)
    ...
    def _docker_stream_to_remote(
        ...
        if save_returncode != 0:
            result = self._docker_error(
                image_name=image_name, tag=tag, image_ref=image_ref,
                archive_path="", remote_archive_path="", command_label=command_label,
                error_code="E_DOCKER_SAVE", stderr=save_stderr,
            )
    ...
    def _dockerized_gradle_build_command(self, project_dir: Path) -> list[str]:
        return [
            ...
            "-Dorg.gradle.native=false", "-x", "test",
        ]
